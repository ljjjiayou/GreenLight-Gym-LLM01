"""Build readiness v33 for near-miss consolidation and existing-cache discovery."""
...
def _summary_pass(summary_audit: Mapping[str, Any] | None) -> bool:
    aggregate = summary_audit.get("aggregate", {}) if isinstance(summary_audit, Mapping) else {}
...
def build_report(
    *,
    ...
    inventory_ready = bool(inventory and inventory.get("existing_cache_inventory_ready", False))
    inventory_candidates = int(_num(inventory.get("candidate_scenario_count") if inventory else 0))
    manifest_ready = bool(manifest and manifest.get("strict_targeted_shadow_sweep_v33_manifest_ready", False))
    cache_ok = _coverage_pass(cache_coverage)
    execution_authorized = bool(execution_record and execution_record.get("strict_targeted_shadow_sweep_v33_authorized", False))
    ...
    if not catalog_ready:
        failure_taxonomy.append("near_miss_catalog_missing")
    if inventory and not inventory_candidates:
        failure_taxonomy.append("existing_cache_exhausted_for_independent_strict_source")
    if cache_coverage and not cache_ok:
        failure_taxonomy.append("cache_mismatch")
    if sourcing_run and not source_found:
        failure_taxonomy.append("strict_targeted_source_not_found_in_existing_cache")
    ...
    if not catalog_ready:
        next_action = "near_miss_source_catalog_required"
    elif not inventory:
        next_action = "existing_cache_source_inventory"
    elif not inventory_candidates:
        next_action = "new_scenario_cache_acquisition_design"
    elif not manifest:
        next_action = "strict_targeted_shadow_sweep_v33_manifest_required"
    elif not manifest_ready:
        next_action = "strict_targeted_shadow_sweep_v33_manifest_blocked"
    elif not cache_coverage:
        next_action = "run_v33_no_fill_cache_coverage_precheck"
    elif not cache_ok:
        next_action = "v33_cache_coverage_blocked"
    elif not execution_record:
        next_action = "v33_shadow_sweep_execution_record_required"
    elif not execution_authorized:
        next_action = "v33_shadow_sweep_not_authorized"
    elif not sourcing_run:
        next_action = "execute_v33_shadow_sweep_and_strict_targeted_sourcing"
    elif post_run_audits_provided and not post_run_audits_pass:
        next_action = "diagnose_v33_post_run_audit_failure"
    elif source_found:
        next_action = "strict_source_manifest_and_minimal_canary_admission_plan"
    else:
        next_action = "new_scenario_cache_acquisition_design"

    return {
        "schema_version": "metadata_replay_readiness_checklist_20260528_v33",
        "mainline_alignment": {
            "affected_layers": ["Evaluation", "Safety Boundary", "RSPC", "Metadata Trace"],
            "default_llm_rspc_v2_changed": False,
            "mode": "v33 near-miss consolidation and existing-cache shadow-only discovery readiness",
        ...
        "existing_cache_inventory_ready": inventory_ready,
        "existing_cache_source_candidates_found": bool(inventory_candidates),
        "v33_manifest_ready": manifest_ready,
        "v33_cache_coverage_pass": cache_ok,
        "v33_shadow_sweep_authorized": execution_authorized,
        "v33_shadow_sweep_executed": bool(execution_authorized and sourcing_run),
        ...
        "metrics": {
            "near_miss_scenario_count": int(_num(near_miss_catalog.get("near_miss_scenario_count"))),
            "near_miss_row_count": int(_num(near_miss_catalog.get("near_miss_row_count"))),
            "inventory_candidate_scenario_count": inventory_candidates,
            ...
            "expected_strict_eligible_applied_steps": int(_num(strict_sourcing.get("expected_strict_eligible_applied_steps") if strict_sourcing else 0)),
...
def build_markdown_report(report: Mapping[str, Any]) -> str:
    lines = [
        "# Metadata Replay Readiness Checklist v33",
        "",
        "- Mode: near-miss consolidation and existing-cache shadow-only discovery",
        "- Metadata replay execution allowed: false",
        "- Controlled replay allowed: false",
        "- Controlled replay execution allowed: false",
        "- Performance claim allowed: false",
        "- Promotion evidence: false",
        f"- Existing-cache candidates found: {report.get('existing_cache_source_candidates_found', False)}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_report(
        near_miss_catalog=_load_optional(args.near_miss_catalog_json) or {},
        inventory=_load_optional(args.inventory_json),
    ...
    print(f"existing_cache_source_candidates_found={report['existing_cache_source_candidates_found']}")
