"""Record authorization for the v30 coverage-qualified shadow-only sweep."""
...
def _command_for_group(
    *,
    ...
    group_id = str(group.get("group_id", "v30"))
    return [
        "python",
        runner_path,
        "--years",
        ...
        "scenario_timestep",
        "--output-json",
        str(Path(output_root) / f"strict_targeted_shadow_source_sweep_v30_{group_id}_20260528.json"),
        "--output-trace-dir",
...
def build_report(
    *,
    ...
    manifest_ready = bool(
        manifest.get("strict_targeted_shadow_sweep_v30_manifest_ready", False)
        and manifest.get("shadow_only", False)
        and int(_num(manifest.get("scenario_count"))) == 28
        and int(_num(manifest.get("blocked_cache_repair_backlog_count"))) == 8
    )
    ...
    return {
        "schema_version": "strict_targeted_shadow_sweep_v30_execution_record_v1",
        "mainline_alignment": {
            "affected_layers": ["Evaluation", "Safety Boundary", "RSPC", "Metadata Trace"],
            "default_llm_rspc_v2_changed": False,
            "mode": "coverage-qualified strict-targeted shadow-only sweep v30",
            "reopens_rejected_preset": False,
        },
        "authorization_source": authorization_source,
        "strict_targeted_shadow_sweep_v30_authorized": authorized,
        "scope": "coverage_qualified_strict_targeted_shadow_only_sweep_v30" if authorized else "blocked",
        ...
        "blocked_cache_repair_backlog": list(manifest.get("blocked_cache_repair_backlog", []) or []),
        "selected_cache_path": selected_cache_path,
        ...
        "precheck": {
            ...
            "scenario_count_is_28": int(_num(manifest.get("scenario_count"))) == 28,
            "blocked_backlog_count_is_8": int(_num(manifest.get("blocked_cache_repair_backlog_count"))) == 8,
        ...
        "next_action": "execute_strict_targeted_shadow_sweep_v30" if authorized else "strict_targeted_shadow_sweep_v30_not_authorized",
    }


def build_markdown_report(report: Mapping[str, Any]) -> str:
    lines = [
        "# Strict-Targeted Shadow Sweep v30 Execution Record",
        "",
        f"- Authorized: {report.get('strict_targeted_shadow_sweep_v30_authorized', False)}",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"strict_targeted_shadow_sweep_v30_authorized={report['strict_targeted_shadow_sweep_v30_authorized']}")
    print(f"next_action={report['next_action']}")
    print(f"wrote {output_json}")
    print(f"wrote {output_md}")
    return 0 if report["strict_targeted_shadow_sweep_v30_authorized"] else 1
